 ...
 * Backstory AI, Download Buttons, and Fixed Audio Functions
 */
...
const IslamicModule = (function () {
  ...
  function setQari(qariId) {
    if (QARI_LIST.some(q => q.id === qariId)) {
      ...
      // Clear cached surah data for the currently open surah (if any)
      // We'll handle this in the UI by reloading the surah
    }
  }
  ...
  async function deleteFromDB(storeName, keyPattern) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          if (cursor.key.toString().startsWith(keyPattern)) {
            cursor.delete();
          }
          cursor.continue();
        } else {
  ...
  const QuranAPI = {
    ...
    // New: fetch backstory using Gemini AI
    getAyahBackstory: async (surah, ayah, arabic, bangla) => {
      const key = localStorage.getItem('geminiKey');
      const model = localStorage.getItem('geminiModel') || 'gemini-1.5-flash';
      if (!key) return "ব্যাকস্টোরি দেখতে জেমিনি API কী দিন।";

      const prompt = `তুমি একজন ইসলামিক পণ্ডিত। নিচের আয়াতটির প্রসঙ্গ (শানে নুযুল) ও সংক্ষিপ্ত তাফসির দাও:
সূরা: ${surah}, আয়াত: ${ayah}
আরবি: ${arabic}
বাংলা অনুবাদ: ${bangla}

উত্তর হবে খুব সংক্ষিপ্ত (সর্বোচ্চ ১৫০ শব্দ) এবং শুধু বাংলায়। কোনো অতিরিক্ত ব্যাখ্যা নয়।`;
      ...
      try {
        ...
        return text || "ব্যাকস্টোরি পাওয়া যায়নি।";
      } catch (e) {
        console.error("Backstory fetch error", e);
        return "ব্যাকস্টোরি লোড করতে সমস্যা হয়েছে।";
      }
    }
  };
  ...
  const HadithAPI = {
    ...
    // Get hadiths from a specific book (for openHadithBook)
    getHadithsByBook: async (bookId, limit = 50) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
          const all = request.result;
          const filtered = all.filter(item => item.key.startsWith(`${bookId}_`)).map(item => item.data);
  ...
  const UIRenderer = {
    ...
    playAudio: (url, btnElement, onEndCallback) => {
      if (UIRenderer.currentAudio) {
        ...
        if (UIRenderer.currentAudioBtn) {
          UIRenderer.currentAudioBtn.classList.remove('audio-playing');
          UIRenderer.currentAudioBtn.innerHTML = '<span class="material-symbols-rounded">play_circle</span> প্লে';
        }
      }
      ...
      if (url) {
        ...
        UIRenderer.currentAudioBtn = btnElement;
        if (btnElement) {
        ...
        UIRenderer.currentAudio.onended = () => {
          ...
          UIRenderer.currentAudioBtn = null;
    ...
    playTTS: (text, btnElement, lang = 'bn-BD', voiceHint = '', onEndCallback) => {
      // We'll use a simple flag to stop (not needed here)
    ...
    buildQuranCard: (ayahData, originalRefText) => {
      ...
      const safeBangla = ayahData.bangla.replace(/'/g, "\\'").replace(/"/g, '&quot;');
      const safeEnglish = ayahData.english.replace(/'/g, "\\'").replace(/"/g, '&quot;');
      const safeArabic = ayahData.arabic.replace(/'/g, "\\'").replace(/"/g, '&quot;');
      return `
      <div class="islamic-embed-card" id="${id}">
          ...
          <div class="embed-footer">
              <button class="embed-btn" onclick="IslamicModule.UI.playAudio('${ayahData.audio}', this, function(){ if(IslamicModule.Quran.getAutoAdvance()) { let next = document.querySelector('#${id}').nextElementSibling; if(next && next.querySelector('.embed-btn')) next.querySelector('.embed-btn').click(); } })">
                  <span class="material-symbols-rounded">play_circle</span> প্লে
              </button>
              ${ayahData.bangla ? `<button class="embed-btn" onclick="IslamicModule.UI.playTTS('${safeBangla}', this, 'bn-BD')"><span class="material-symbols-rounded">record_voice_over</span> বাংলা অডিও</button>` : ''}
              ${ayahData.english ? `<button class="embed-btn" onclick="IslamicModule.UI.playTTS('${safeEnglish}', this, 'en-US')"><span class="material-symbols-rounded">record_voice_over</span> English Audio</button>` : ''}
              ...
              <button class="embed-btn" onclick="IslamicModule.UI.showBackstory(${ayahData.surah}, ${ayahData.ayah}, '${safeArabic}', '${safeBangla}', this)">
                  <span class="material-symbols-rounded">history_edu</span> ব্যাকস্টোরি
              </button>
              <button class="embed-btn" onclick="AudioManager.download('${ayahData.surah}', ${ayahData.ayah}, this)">
                  <span class="material-symbols-rounded">download_for_offline</span> ডাউনলোড
              </button>
          </div>
      </div>`;
    },
    ...
    showBackstory: async (surah, ayah, arabic, bangla, btn) => {
      ...
      try {
        const story = await IslamicModule.Quran.getAyahBackstory(surah, ayah, arabic, bangla);
        // Create a small modal or tooltip
        ...
        modal.style.border = '2px solid var(--accent-primary)';
        modal.innerHTML = `
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
            <h3 style="color:var(--accent-primary);">আয়াতের প্রসঙ্গ</h3>
            <button class="icon-btn" onclick="this.parentElement.parentElement.remove()"><span class="material-symbols-rounded">close</span></button>
          </div>
          <div style="font-size:16px; line-height:1.6;">${story}</div>
          <button class="embed-btn" style="margin-top:16px; width:100%;" onclick="this.parentElement.remove()">বন্ধ করুন</button>
        `;
        document.body.appendChild(modal);
      } catch (e) {
        alert("ব্যাকস্টোরি লোড করতে সমস্যা হয়েছে।");
      } finally {
        btn.disabled = false;
